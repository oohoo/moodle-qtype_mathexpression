 ...
 * @param height the height of the icon (by default it is 25)
 */
 MathEditor.ButtonCommand = function(name, latex, xcoord, ycoord, width, height) {
    this.name = name;
    this.latex = latex;
    this.cmd = latex;
    this.xcoord = xcoord;
 ...
 * @param height the height of the icon (by default it is 25)
 */
 MathEditor.BC = function(name, latex, xcoord, ycoord, width, height) {
    return new MathEditor.ButtonCommand(name, latex, xcoord, ycoord, width, height);
};
        ...
        $(tab.buttons).each(function(buttonIndex, button) {
            ...
            button.dom.click(function(e) {
                if (button.matrix) {
                    ...
                    if (button.cmd != null) {
                        self.equation.mathquill('selectPrevBlock');
 ...
 */
 MathEditor.prototype.getContent_ = function() {
    ...
    jQuery.each(actions, function(i){
        var elem = actions[i];
        var coord = jQuery.parseJSON(elem.coordinates);
        category1_elems.push(MathEditor.BC('matheditor.' + elem.name, '\\' + elem.name, coord.x, coord.y));
    });
    ret.push(MathEditor.T('matheditor.general', category1_elems));
    ...
    MathEditor.T('matheditor.miscellaneous', [
        ...
        MathEditor.BC('matheditor.meCrossProduct', '\\meCrossProduct', 100, 100),
        MathEditor.BC('matheditor.meDotProduct', '\\meDotProduct', 25, 0),
        MathEditor.BC('matheditor.meScalarProduct', '\\meScalarProduct', 200, 150),
        MathEditor.BC('matheditor.mePower', '\\mePower', 50, 25),
        ])
    ];
};
